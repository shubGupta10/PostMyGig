...
import { Calendar, Clock, User, DollarSign, AlertCircle, RefreshCw, CheckCircle, ArrowLeft, Share2, Bookmark, Eye, MapPin, Globe } from 'lucide-react'
  ...
  const fetchGig = async () => {
    try {
      ...
      if (response.ok && data.gig) {
        setGig(data.gig);
        console.log('Gig set successfully:', data.gig);
      } else {
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            <div className="relative mb-8">
              ...
              <div className="absolute inset-0 rounded-full h-16 w-16 border-4 border-transparent border-t-emerald-300 animate-spin mx-auto" style={{ animationDelay: '0.3s', animationDuration: '1.5s' }}></div>
            </div>
      ...
      <div className={`bg-gradient-to-r ${statusConfig.bgGradient} border-b border-slate-200`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <button onClick={() => router.back()} className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 transition-colors font-medium">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-8">
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-4 mb-4">
                ...
                {gig.isFlagged && (
                  <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200 ring-orange-100 ring-1 font-semibold">
                    🚩 Flagged
                  </Badge>
                )}
                {isExpiringSoon && (
                  <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200 ring-amber-100 ring-1 font-semibold animate-pulse">
                    ⏰ Expiring Soon
                  </Badge>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-6 text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">ID: {gig.createdBy}</span>
                </div>
              </div>
            </div>
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between py-3">
                    ...
                    <Badge
                      variant="outline"
                      className={`${statusConfig.color} ring-1 font-semibold`}
                    >
                ...
                <div className="mt-8 space-y-4">
                  <button className="w-full inline-flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-xl font-bold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                    <CheckCircle className="w-5 h-5" />
                    Apply for this Gig
                  </button>

                  {userId == gig.createdBy ? <>
                    <button onClick={() => router.push(`/edit-gig/${gig._id}`)} className="w-full inline-flex items-center justify-center gap-3 px-6 py-4  text-emerald-600 border-2 border-emerald-600 rounded-xl font-bold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                      <CheckCircle className="w-5 h-5" />
                      Edit Gig
                    </button>
                  </> : null}
