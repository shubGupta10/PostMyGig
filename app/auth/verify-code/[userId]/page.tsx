      ...
      <div className="w-full max-w-lg bg-card rounded-3xl shadow-2xl overflow-hidden border border-border">
        ...
        <div className="bg-primary px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6 text-primary-foreground">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-foreground/20 rounded-lg sm:rounded-xl flex items-center justify-center backdrop-blur-sm">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-primary-foreground/80">Enter the 6-digit code we sent you</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          {/* Timer */}
          <div className="flex items-center justify-center gap-2 mb-6 p-2 sm:p-3 bg-muted rounded-lg sm:rounded-xl">
            <Clock className="w-4 h-4 text-muted-foreground" />
            <span className="text-xs sm:text-sm text-muted-foreground">
              Code expires in:{" "}
              <span className="font-mono font-semibold text-accent-foreground">{formatTime(timeLeft)}</span>
            </span>
          </div>
          ...
          {error && (
            <Alert
              variant="destructive"
              className="mb-6 border-destructive/20 bg-destructive/10 animate-in fade-in-50 duration-300 rounded-lg sm:rounded-xl"
            >
              <AlertCircle className="h-4 sm:h-5 w-4 sm:w-5 text-destructive" />
              <AlertDescription className="text-destructive ml-2 font-medium text-sm sm:text-base">
                {error}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex justify-center px-2 gap-2 sm:gap-3 mb-6 sm:mb-8" onPaste={handlePaste}>
            {verifyCode.map((digit, index) => (
              <input
                ...
                className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 text-center text-lg sm:text-xl md:text-2xl font-bold border-2 border-border rounded-lg sm:rounded-xl focus:border-ring focus:ring-ring focus:outline-none transition-all duration-200 disabled:bg-muted disabled:cursor-not-allowed bg-background text-foreground"
                maxLength={1}
                disabled={isLoading}
                autoComplete="off"
                inputMode="numeric"
          ...
          >
            {isLoading ? (
              <div className="flex items-center gap-3">
                <div className="w-4 sm:w-5 h-4 sm:h-5 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin"></div>
                Verifying...
              </div>
            ) : (
              <div className="flex items-center gap-3">
          ...
          <div className="text-center">
            <p className="text-sm sm:text-base text-muted-foreground mb-3">Didn't receive the code?</p>
            <Button
              onClick={handleResendCode}
              variant="outline"
              className="border-2 border-border hover:border-primary/30 hover:bg-primary/5 text-foreground font-semibold px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base rounded-lg sm:rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isResending || resendCooldown > 0 || timeLeft === 0}
            >
              {isResending ? (
                <div className="flex items-center gap-2">
                  <div className="w-3 sm:w-4 h-3 sm:h-4 border-2 border-muted-foreground border-t-transparent rounded-full animate-spin"></div>
                  Sending...
                </div>
              ) : resendCooldown > 0 ? (
                <div className="flex items-center gap-2">
          ...
          {/* Help Text */}
          <div className="mt-6 sm:mt-8 p-3 sm:p-4 bg-accent/20 border border-accent/30 rounded-lg sm:rounded-xl">
            <div className="flex gap-2 sm:gap-3">
              <div className="w-5 h-5 rounded-full bg-primary flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-primary-foreground text-xs font-bold">?</span>
              </div>
              <div className="text-xs sm:text-sm text-accent-foreground">
