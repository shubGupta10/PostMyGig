...
import { Mail, Eye, Check, Calendar, Sparkles, CheckCircle2, MessageSquare } from "lucide-react"
...
export function ApplicationCard({
  ...
  return (
    <div className="p-5 sm:p-6 bg-card transition-colors">
      {/* Main Row */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        {/* Left: Avatar & Identity & Skills */}
        <div className="flex items-start sm:items-center gap-4 min-w-0 flex-1">
          <div className="relative shrink-0">
            <div className="w-13 h-13 rounded-2xl border-2 border-primary bg-primary text-primary-foreground flex items-center justify-center overflow-hidden font-bold text-lg">
          ...
          <div className="min-w-0 space-y-1">
            ...
            <div className="flex items-center gap-4 text-xs sm:text-sm flex-wrap text-muted-foreground">
        ...
        <div className="flex items-center gap-2.5 self-end lg:self-center w-full lg:w-auto mt-2 lg:mt-0 shrink-0">
          <Button
            ...
            className="flex-1 lg:flex-none h-10 px-4 rounded-xl font-semibold text-sm cursor-pointer border border-border hover:bg-muted bg-background"
          >
          ...
          {!isAccepted ? (
            ...
            <Button
              size="sm"
              onClick={() => (window.location.href = `/projects/${applicant.projectId}/huddle`)}
              className="flex-1 lg:flex-none h-10 px-5 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:opacity-90 transition-opacity cursor-pointer"
            >
              <MessageSquare className="w-4 h-4 mr-1.5" /> Open Project Huddle
            </Button>
          )}
        </div>
      </div>
