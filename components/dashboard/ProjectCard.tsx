...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <Card className="flex flex-col h-full border border-border bg-card shadow-sm hover:shadow-md transition-shadow duration-200 rounded-xl overflow-hidden">
      <CardHeader className="p-6 pb-4 space-y-3 flex-none relative">
        <div className="flex items-start justify-between gap-4">
          <CardTitle className="text-lg font-semibold text-primary line-clamp-2 leading-tight">
            {project.title}
          </CardTitle>
          <Badge className={`${getStatusColor(project.status)} capitalize text-xs font-semibold px-3 py-1 rounded-full shrink-0 shadow-xs`}>
            {project.status}
          </Badge>
        </div>
      ...
      <CardFooter className="p-6 pt-5 flex-none mt-auto border-t border-border/50">
        <Button
          onClick={() => router.push(`/open-gig/${project._id}`)}
          className="w-full justify-center h-10 bg-secondary text-secondary-foreground hover:opacity-90 font-semibold text-sm transition-all rounded-lg shadow-sm"
        >
...
export function EmptyState({ message = "No projects found" }: { message?: string }) {
  ...
  return (
    <Card className="border border-dashed border-border bg-card p-12 text-center rounded-xl flex flex-col items-center justify-center">
      <div className="w-16 h-16 bg-secondary text-secondary-foreground rounded-2xl flex items-center justify-center mb-6 shadow-xs">
        <Briefcase className="h-8 w-8" />
      </div>
      <h3 className="text-lg font-semibold text-foreground mb-2">{message}</h3>
      <p className="text-sm text-foreground opacity-80 font-normal max-w-sm mb-8 leading-relaxed">
        Create your first gig to start receiving applications from professionals.
      </p>
      <Button
        onClick={() => router.push("/add-gigs")}
        className="h-11 bg-primary text-primary-foreground font-semibold text-sm px-8 rounded-xl transition-colors shadow-sm"
      >
